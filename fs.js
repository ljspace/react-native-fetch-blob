...
function createFile(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (encoding.toLowerCase() === "ascii") {
      if (Array.isArray(data)) RNFetchBlob.createFileASCII(path, data, handler);
      else
        reject(
          new Error("`data` of ASCII file must be an array contains numbers")
        );
    } else {
  ...
  if (encoding.toLocaleLowerCase() === "ascii") {
    if (!Array.isArray(data))
      return Promise.reject(
        new Error(
          `Expected "data" is an Array when encoding is "ascii", however got ${typeof data}`
        )
      );
    ...
    if (typeof data !== "string")
      return Promise.reject(
        new Error(
          `Expected "data" is a String when encoding is "utf8" or "base64", however got ${typeof data}`
        )
      );
...
function isDir(path: string): Promise<boolean, boolean> {
  return new Promise((resolve, reject) => {
    try {
